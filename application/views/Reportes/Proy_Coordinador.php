...
{
    $name = "SELECT * FROM VW_REPORTE_PROYECTOS_COORDINADOR WHERE ID_COORDINADOR = $coordinadorR;";
    ...
    if ($resp > 0)
    {
        ...
        $pdf->Cell(195, 6, "REPORTE DE PROYECTOS POR COORDINADOR", 0, 1, "C");
        while ($fila = $resultado_n->fetch_assoc())
        {
            $pdf->Ln(5);
            $pdf->SetFont("Times", "B", 18);
            $pdf->Cell(195, 6, utf8_decode($fila['COORDINADOR']), 0, 1, "C");
        }
...
{
    $this->session->set_flashdata('msjerror', '!Seleccione un coordinador!');
    redirect('/Proyectos/');
}
?>